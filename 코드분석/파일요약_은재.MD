...
## core/src/main/java/com/graphhopper/reader/dem : dem 폴더는 GraphHopper에서 고도 데이터를 처리하고 제공하는 기능을 담당하는 클래스들의 집합입니다. 이 폴더의 클래스들은 다양한 고도 데이터 소스(SRTM, GMTED, Skadi 등)를 읽고, 다운로드 및 캐싱하며, 이를 기반으로 엣지와 노드의 고도 정보를 보간, 샘플링, 평활화하여 라우팅 시 필요한 고도 프로파일을 생성하는 역할을 합니다.
