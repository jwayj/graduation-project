...
public class RoutingExample {
    public static void main(String[] args) {
        ...
        //route with circle
        ResponsePath circularPath = createCircularRoute(hopper, start, desiredDistance);
        if (circularPath != null) {
            System.out.println("순환 경로 거리: " + circularPath.getDistance() + " 미터");
            
            String geoJson = GeoJsonExporter.toGeoJSON(circularPath);
            System.out.println("GeoJSON:\n" + geoJson);
            try {
                SaveGeoJson.saveToFile(geoJson, "circular_route.geojson");
                System.out.println("GeoJSON saved to circular_route.geojson");
            } catch (Exception e) {
                System.err.println("Error saving GeoJSON: " + e.getMessage());
            }
        } else {
            System.out.println("순환 경로를 찾을 수 없습니다.");
        }
    ...
    public static ResponsePath createCircularRoute(GraphHopper hopper, GHPoint start, double desiredDistance) {
        double radius = desiredDistance / (2 * Math.PI);
        List<GHPoint> circlePoints = new ArrayList<>();
    
        for (int i = 0; i < 12; i++) {
            double angle = Math.toRadians(i * 30);
            double lat = start.lat + (radius / 111000) * Math.cos(angle);
            double lon = start.lon + (radius / (111000 * Math.cos(Math.toRadians(start.lat)))) * Math.sin(angle);
            circlePoints.add(new GHPoint(lat, lon));
        }
        ...
        for (GHPoint point : circlePoints) {
            ResponsePath pathToPoint = findPath(hopper, start, point, "foot");
            ResponsePath pathBack = findPath(hopper, point, start, "foot");
    
            if (pathToPoint != null && pathBack != null) {
                double totalDistance = pathToPoint.getDistance() + pathBack.getDistance();
                double difference = Math.abs(totalDistance - desiredDistance);
    
                if (!usesUsedEdges(pathToPoint, usedEdges) && !usesUsedEdges(pathBack, usedEdges)) {
                    if (difference < closestDistance) {
                        closestDistance = difference;
                        bestPath = combinePaths(pathToPoint, pathBack);
                    }
                }
            }
        }
    
        if (bestPath.getDistance()>desiredDistance-100 && bestPath.getDistance() < desiredDistance + 100) {
            updateUsedEdges(bestPath, usedEdges);
            return bestPath;
        } else {
            System.out.println("경로를 생성할 수 없습니다.");
            return null;
        }
    }
    
    
    private static boolean usesUsedEdges(ResponsePath path, Set<Long> usedEdges) {
        Map<String, List<PathDetail>> pathDetails = path.getPathDetails();
        List<PathDetail> edgeIdDetails = pathDetails.get(Parameters.Details.EDGE_ID);
    
        if (edgeIdDetails != null) {
            for (PathDetail detail : edgeIdDetails) {
                Long edgeId = Long.valueOf(detail.getValue().toString()); // Object를 Long으로 변환
    ...
    private static void updateUsedEdges(ResponsePath path, Set<Long> usedEdges) {
        Map<String, List<PathDetail>> pathDetails = path.getPathDetails();
        List<PathDetail> edgeIdDetails = pathDetails.get(Parameters.Details.EDGE_ID);
