...
public class MapMatching {
    ...
    //새로운 경로 알고리즘
    public List<Path> findPathsWithExactDistance(int start, int end, double targetDistance) {
        PriorityQueue<PathNode> queue = new PriorityQueue<>();
        queue.add(new PathNode(0, start, Arrays.asList(start)));
        ...
        while (!queue.isEmpty()) {
            ...
            if (currentNodeId == end && currentDistance >= targetDistance) {
                validPaths.add(new Path(currentPath, currentDistance));
            }

            for (Edge edge : graph.getEdges(currentNodeId)) {
                int nextNode = edge.getNeighbor(currentNodeId);
                int newDistance = currentDistance + edge.getDistance();

                if (newDistance <= targetDistance || nextNode == end) {
                    List<Integer> newPath = new ArrayList<>(currentPath);
                    newPath.add(nextNode);
                    queue.add(new PathNode(newDistance, nextNode, newPath));
                }
            }
        }

        if (validPaths.isEmpty()) {
            System.out.println("목표 거리를 충족하는 경로가 없습니다.");
            return null;
        }
    ...
    private class PathNode implements Comparable<PathNode> {
        ...
        PathNode(int distance, int node, List<Integer> path) {
    ...
    // 기존 computePaths 메서드를 대체하여 새로운 경로 탐색 알고리즘을 호출합니다.
    public MatchResult match(List<Observation> observations, double targetDistance) {
        List<Path> paths = findPathsWithExactDistance(
                observations.get(0).getPoint().getNode(),  // 시작 노드
                observations.get(observations.size() - 1).getPoint().getNode(),  // 끝 노드
                targetDistance);

        if (paths == null || paths.isEmpty()) {
            System.out.println("매칭된 경로가 없습니다.");
            return null;
        }

        // 반환된 paths를 이용하여 MatchResult 생성
        MatchResult result = new MatchResult(paths);
